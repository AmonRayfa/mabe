...
pub(super) fn find_active_left_brace<M: ToString>(msg: M, start: usize) -> Option<usize> {
    ...
    match msg[start..].find('{') {
        Some(mut i) => {
            ...
            if consec_left_braces % 2 != 0 { Some(i) } else { find_active_left_brace(msg, i + 1) }
        }
        None => None,
    }
}
...
pub(super) fn find_active_right_brace<M: ToString>(msg: M, start: usize) -> Option<usize> {
    let msg = msg.to_string();
    ...
    match msg[start..].find('}') {
        Some(mut i) => {
            ...
            if consec_right_braces % 2 != 0 { Some(index) } else { find_active_right_brace(msg, i + 1) }
        }
        None => None,
    }
}
