...
pub(super) fn format_msg<M: ToString>(msg: M) -> (String, Vec<String>) {
    ...
    while checkpoint < msg.len() {
        ...
        if let (Some(idx_lb), Some(idx_rb)) = (index_left_brace, index_right_brace) {
            if idx_lb < idx_rb {
                formatted_msg.push_str(&msg[checkpoint..idx_lb]);
                formatted_msg.push_str(generic_placeholder.as_str());
                extracted_args.push(msg[idx_lb + 1..idx_rb].to_string());
                placeholder_position += 1;
                checkpoint = idx_rb + 1;
            } else {
                formatted_msg.push_str(&msg[checkpoint..idx_rb]);
                formatted_msg.push_str(generic_placeholder.as_str());
                extracted_args.push("}".to_string());
                placeholder_position += 1;
                checkpoint = idx_rb + 1;
            }
        } else if let (Some(idx_lb), None) = (index_left_brace, index_right_brace) {
            formatted_msg.push_str(&msg[checkpoint..idx_lb]);
            formatted_msg.push_str(generic_placeholder.as_str());
            extracted_args.push("{".to_string());
            placeholder_position += 1;
            checkpoint = idx_lb + 1;
        } else if let (None, Some(idx_rb)) = (index_left_brace, index_right_brace) {
            formatted_msg.push_str(&msg[checkpoint..idx_rb]);
            formatted_msg.push_str(generic_placeholder.as_str());
            extracted_args.push("}".to_string());
            placeholder_position += 1;
...
/// [`Vec<TokenStream>`](https://docs.rs/proc-macro2/latest/proc_macro2/struct.TokenStream.html) types. The pattern bindings are
/// the fields of an enum variant and the keyword arguments are the placeholders and the extracted arguments. The `dunder`
/// parameter is a boolean that determines whether the pattern bindings should have underscores at the beginning and end.
pub(super) fn map_args<A: ToString, F: ToString>(
    args: &[A],
    fields: &[F],
    dunder: bool,
